...
module.exports.getAdvSlots = function(req, res){
  ...
  AdvSlot.find({},{name: 1, price: 1, length: 1, width: 1},function(err, advSlot){
    // If there is an error return it in response
    if (err) {
      return res.json({success: false, msg: "Error retrieving advSlots", error: err});
    }
    else{
      //returns an array of AdvSlots
      res.json({success: true, msg: 'Successful retrieval', advSlot});
    }
  })
};
    ...
    const newAdvBooking = new AdvBooking({
     ...
     if (err) return res.json({success: false, msg: 'Error adding the booking', error: err});
      ...
      AdvSlot.findByIdAndUpdate(
        booking.advSlot,
        {$push:{"advSchedule": booking._id}},
        {safe: true, upsert: true, new: true},
        function(err, advSlot){
          // If there is an error return it in response
          if(err) res.json({success: false, msg: "Error updating Adv Slot", error: err});
        }
      );
     })
    };
